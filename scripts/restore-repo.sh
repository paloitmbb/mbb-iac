...
# The bare repo was cloned with --mirror, which sets remote.origin.mirror=true.
# That flag conflicts with explicit refspecs, so unset it first.
# We push only refs/heads/* and refs/tags/* — excluding refs/pull/* which
# GitHub blocks as "hidden refs".
REMOTE_URL="https://x-access-token:${GH_BACKUP_TOKEN}@github.com/${GITHUB_ORG}/${TARGET_REPO}.git"

cd "${BARE_REPO_DIR}"

git remote set-url origin "${REMOTE_URL}"
git config --unset remote.origin.mirror || true   # unset mirror flag if present
...
echo "Branches to push: ${BRANCH_COUNT} | Tags to push: ${TAG_COUNT}"

if [[ "$BRANCH_COUNT" -gt 0 ]]; then
  echo "Pushing branches to ${GITHUB_ORG}/${TARGET_REPO}..."
  if ! git push origin 'refs/heads/*:refs/heads/*' --force 2>&1; then
    echo "❌ git push branches failed."
    exit 1
  fi
  echo "Branches pushed successfully."
else
  echo "No branches found in backup — skipping branch push."
fi

if [[ "$TAG_COUNT" -gt 0 ]]; then
  echo "Pushing ${TAG_COUNT} tag(s)..."
  if ! git push origin 'refs/tags/*:refs/tags/*' --force 2>&1; then
