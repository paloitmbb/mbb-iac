...
# Production environment for GitHub repository management
...
terraform {
  ...
  #   key            = "github-repos/prod/terraform.tfstate"
...
locals {
  ...
  repositories = {
    ...
    if try(repo.environment, "all") == "production" || try(repo.environment, "all") == "all"
  }
}
...
locals {
  ...
  organizations = {
    ...
    if(try(org.environment, "all") == "production" || try(org.environment, "all") == "all") && var.enterprise_slug != ""
  }
}
