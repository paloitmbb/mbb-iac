...
# Pre-production environment for GitHub repository management
...
terraform {
  ...
  #   key            = "github-repos/staging/terraform.tfstate"
...
locals {
  ...
  repositories = {
    ...
    if try(repo.environment, "all") == "staging" || try(repo.environment, "all") == "all"
  }
}
...
locals {
  ...
  organizations = {
    ...
    if(try(org.environment, "all") == "staging" || try(org.environment, "all") == "all") && var.enterprise_slug != ""
  }
}
